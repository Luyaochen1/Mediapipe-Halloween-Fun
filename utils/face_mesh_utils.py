...
class ExorcistFace():
    ...
    def read_exorcist_image(self, exorcist_image_url):
        if 'http' in exorcist_image_url:
            self.exorcist_image = imread_from_url(exorcist_image_url)
        else:	
            self.exorcist_image = cv2.imread(exorcist_image_url) #, cv2.IMREAD_GRAYSCALE)
    ...
    def detect_exorcist_mesh(self):
        ...
        self.exorcist_face_coordinates = np.array([[int(landmark.x*self.ex_im_width), int(landmark.y*self.ex_im_height)] for landmark in landmarks[0].landmark])
    ...
    def draw_exorcist_face(self, img, landmarks):
        ...
        for face_landmarks in landmarks:
            face_coordinates = np.array([[int(min([landmark.x*img.shape[1],img.shape[1]-1])), int(min([landmark.y*img.shape[0],img.shape[0]-1]))] for landmark in face_landmarks.landmark]).astype(int)
            ...
            for triangle_id in range(0, len(vertices), 3):
                ...
                warp_mat = cv2.getAffineTransform(exorcist_pix_crop_coords.astype(np.float32), face_pix_crop_coords.astype(np.float32))
